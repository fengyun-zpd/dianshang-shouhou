...
> 文档更新：2026-09-16。记录业务可靠性问题与对应防护，历史验证日期见测试基线。
...
验证规则：全量回归、黄金集及业务场景验证分别记录在 `TESTING_BASELINE.md` 与运行输出中。PostgreSQL
...
完整历史复现和实测数字见 [补充审查](./RELIABILITY_REVIEW_2026-09-13.md)。

## 审计事件持久标识

迁移 `0006` 增加 `event_id` 并为历史行回填标识，新事件写入持久 ID。PG 接入回归新增了
脱敏、同租户线程审计隔离、未知结果对账后收尾 3 项（该文件现有 7 项）。
旧领域对象缺少 ID 时仍有兼容回退，不能声称所有审计来源均已取消位置标识。
最近已记录的全量结果见[测试基线](./TESTING_BASELINE.md)。
