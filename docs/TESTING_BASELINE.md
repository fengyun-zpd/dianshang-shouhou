...
> **2026-09-13 整改验收已完成。** 修复 R1–R5 后，离线全量 **494 passed / 49 skipped**，
> 隔离 PG 全量 **543 passed / 0 skipped**，边界脚本 R1–R4 全部 PASS；重启回归由两个独立
> Python 进程执行。下方早期 490/538/539 数字与 6/7 FAIL 仍保留为历史审查证据，不能改写为
> 当前结果，详见 [V1.2 补充审查](./V1_2_REVIEW_2026-09-13.md)。
...
# → 494 passed，49 skipped，1 条第三方弃用警告（整改后实测）
...
.venv\Scripts\python.exe -m pytest tests -q
# → 543 passed，0 skipped，1 条第三方弃用警告（整改后实测）
...
不能与隔离结果混用。**历史轮次数字以及整改前的 490/538/539 记录属于当时提交的实测证据，
已被本轮取代，不得再作为当前基线引用。**
...
| 文件 | 项数 | 覆盖 |
| --- | --- | --- |
| `tests/e2e/test_agent_http_lifecycle.py` | 30 | start/clarify/decision/state 正常与失败路径；身份边界（客户 403）；`extra="forbid"`（tenant_id/金额/角色/审批/外部结果 → 422）；跨租户统一 404 且不泄露租户；同名线程跨租户不冲突；SYSTEM 写 unknown 后 decision 重读；伪造 decision 422；线程冲突 409；reset profile 边界；503；循环错误码；HTTP 公共视图和错误响应 PII 脱敏 |
| `tests/integration/test_agent_restart_recovery_live.py` | 3（PG live） | 两个独立 Python 进程 A/B 中断、租约按数据库时间到期、同一 PG + 固定 checkpoint 恢复；错误租户 404；同名线程跨租户；租约未过期时拒绝推进 |
