...
当前已有本地合成数据演示工作台（案件处置 / Agent Lab / 评测与安全），用于展示边界和轨迹；它不是生产后台，且 Agent 启动、澄清和恢复仍由脚本/回放入口驱动。案件页可重复演示审批拒绝、unknown 原键对账、幂等与越权拒绝；Agent Lab 另有隔离固定种子的澄清、无适用政策、跨租户和 PII 脱敏剧本，返回真实 outcome/错误码并显示零退款副作用。若继续，建议按价值排序：① 为现有工作台补浏览器 e2e 与真实 HTTP Agent 主链路（需先补 `start / clarify-resume / state` 接口）；
