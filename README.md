...
- 回归：`.venv` 下 `python -m pytest tests/` → **182 passed**；`python scripts/run_tests.py` → `REGRESSION PASS`；黄金集 `.venv\Scripts\python.exe evals\replay.py` → 11/11；影子 `.venv\Scripts\python.exe evals\run_model_shadow_eval.py --mode offline` → 意图准确率 1.0；A/B `.venv\Scripts\python.exe evals\compare_agents.py` → 单 Agent 与 Supervisor 均 11/11（默认单 Agent）。

### 13.2 规划中（未实现，不写成已实现）

- 阶段 6 Mule Agent Bridge（ADR-003 提议状态）；Supervisor 的 LLM 子 Agent 路由（需重跑 5B 对照、正收益再切默认）、LoRA/QLoRA/DPO 微调对照（无对照数据禁止）、CrewAI（需可量化协作收益）、Graphiti/Neo4j 长期记忆；控制台 / 前端亦规划中。
